...
class Solution:
    # Merge `n` accounts and `e` email occurrences in O(e log e) time and O(n + e) space.
    # NeetCode expresses this using `k` as the maximum emails per account:
    #   O((n * k) log(n * k)) time and O(n * k) space.
    def accountsMerge(self, accounts: List[List[str]]) -> List[List[str]]:
        dsu = DSU(len(accounts))
        origins: dict[str, int] = {}

        # Merge accounts that share an email in the DSU.
