...
class Solution:
    # Merge `n` accounts with `e` total email occurrences in O(e log e) time and O(n + e) space.
    # NeetCode uses `k` for emails per account, giving O((n * k) log(n * k)), where e = O(n * k).
